...
create policy company_iso on job_form_configs using (exists (select 1 from jobs j where j.id = job_form_configs.job_id and j.company_id = (auth.jwt() ->> 'company_id')::uuid)) with check (exists (select 1 from jobs j where j.id = job_form_configs.job_id and j.company_id = (auth.jwt() ->> 'company_id')::uuid));
...
create policy job_form_configs_write on job_form_configs for all using (auth.jwt() ->> 'company_role' in ('admin','recruiter','manager') and exists (select 1 from jobs j where j.id = job_form_configs.job_id and j.company_id = (auth.jwt() ->> 'company_id')::uuid));
...
create policy read_all on job_form_configs for select using (exists (select 1 from jobs j where j.id = job_form_configs.job_id and j.company_id = (auth.jwt() ->> 'company_id')::uuid));
