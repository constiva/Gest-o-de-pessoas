...
- **job_metrics**: estatísticas agregadas por vaga (ex.: cliques no link e `closing_time` com o tempo de fechamento da vaga quando o primeiro candidato entra na última etapa). O tempo de fechamento da vaga exibido nas métricas é calculado a partir de `jobs.created_at`.
