    ...
    if (groupBy) {
      ...
      const rows = ranges.map((r) => {
        ...
        gs.forEach((g) => {
          ...
          const hires = empsG.filter((e) => {
            const hd = e.hire_date ? parseDate(e.hire_date) : null;
            return hd && hd >= r.start && hd <= r.end;
          }).length;
          const terms = empsG.filter((e) => {
            const td = e.termination_date ? parseDate(e.termination_date) : null;
            return td && td >= r.start && td <= r.end;
          }).length;
          const head = empsG.filter((e) => isActive(e, r.start)).length;
          const val = head ? ((hires + terms) / 2 / head) * 100 : 0;
      ...
      const rows = ranges.map((r) => {
        ...
        const head = emps.filter((e) => isActive(e, r.start)).length;
        const val = head ? ((hires + terms) / 2 / head) * 100 : 0;
        return {
