...
export default function CompanyUsersPage() {
  ...
  const openAccessModal = (u: CompanyUser) => {
    setAccessUser(u);
    setScopesText(JSON.stringify(u.scopes || {}, null, 2));
    setAllowedText((u.allowed_fields || []).join(','));
  };

  const saveAccess = async () => {
    if (!accessUser) return;
    try {
      const scopes = scopesText ? JSON.parse(scopesText) : {};
      const allowed_fields = allowedText
        ? allowedText.split(',').map((f) => f.trim()).filter(Boolean)
        : [];
      await fetch('/api/company-users', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
    ...
    <Layout>
      ...
      {accessUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-4 rounded w-full max-w-lg space-y-2">
            <h2 className="text-xl font-semibold mb-2">Configurar acesso</h2>
            <textarea
              className="w-full border p-2 rounded h-40"
              value={scopesText}
              onChange={(e) => setScopesText(e.target.value)}
              placeholder="scopes em JSON"
            />
            <input
              className="w-full border p-2 rounded"
              value={allowedText}
              onChange={(e) => setAllowedText(e.target.value)}
              placeholder="allowed fields separados por vírgula"
            />
