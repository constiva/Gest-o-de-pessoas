  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const statsByName: Record<string, { total: number; count: number; slaHits: number; slaTotal: number; sla: number | null; position: number } > = {};
        (dateRows || []).forEach((r: any) => {
          ...
          const st = statsByName[name] || { total: 0, count: 0, slaHits: 0, slaTotal: 0, sla, position };
          st.total += duration;
          st.count++;
          if (sla) {
            st.slaTotal++;
          ...
          {data.length ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={data} layout="vertical" margin={CHART_MARGIN}>
                ...
                >
                  <LabelList dataKey="value" content={StageLabel} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : (
      ...
      <Card className="h-full">
        ...
        <div ref={pieRef} className="p-4">
          {sourceData.length ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie dataKey="value" data={sourceData} cx="50%" cy="50%" outerRadius={100} label>
                  {sourceData.map((_, i) => (
        ...
        <div className="p-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Card className="md:col-span-2">
              ...
              {radarData.length ? (
                <ChartContainer ref={radarRef} className="p-4">
                  <RadarChart outerRadius={120} data={radarData} margin={{ top: 40, right: 40, bottom: 40, left: 40 }}>
                    <PolarGrid />
                    <PolarAngleAxis
                      dataKey="stage"
                    ...
                    />
                    <PolarRadiusAxis angle={30} domain={[0, maxAvg]} tick={false} />
...
function StageLabel({ value, viewBox, payload }: any) {
  ...
  return (
    ...
    >
      {payload.name}
      <tspan x={centerX} dy="1.2em" className="text-xs font-normal">
        {value}
      </tspan>
    </text>
  );
