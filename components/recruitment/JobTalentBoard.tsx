...
export default function JobTalentBoard({ jobId }: Props) {
  ...
  const load = async () => {
    ...
    const { data: appData } = await supabase
      .from('applications')
      .select(
        'id,stage_id,talent:talents(id,name,created_at,talent_tag_map(tag:talent_tags(name)))'
      )
      .eq('company_id', compId)
      .eq('job_id', jobId);
    const mapped =
      appData?.map((a: any) => ({
        ...
        tags: a.talent.talent_tag_map?.map((m: any) => m.tag.name) || [],
      })) || [];
    setItems(mapped);
  };
          ...
          >
            ...
            {items.map((t) => (
              ...
              >
                ...
                <div className="text-xs text-gray-500">{timeSince(t.created_at)}</div>
                {t.tags.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {t.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 bg-purple-100 rounded text-xs"
                      >
                        {tag}
                      </span>
                    ))}
