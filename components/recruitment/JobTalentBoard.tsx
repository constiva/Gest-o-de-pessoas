...
export default function JobTalentBoard({ jobId }: Props) {
  ...
  const [finalStageId, setFinalStageId] = useState<string | null>(null);
  ...
  const load = async () => {
    ...
    const { data: job } = await supabase
      ...
      .eq('id', jobId)
      .maybeSingle();
    setJobCreatedAt(job?.created_at || null);
    ...
    setFinalStageId(
      stagesData && stagesData.length ? stagesData[stagesData.length - 1].id : null,
    );
  ...
  const onDrop = async (stageId: string) => {
    ...
    if (error) {
      ...
      if (finalStageId && jobCreatedAt) {
        if (stageId === finalStageId) {
          const closing =
            (new Date(now).getTime() - new Date(jobCreatedAt).getTime()) /
            86400000;
          await supabase
            .from('job_metrics')
            .upsert({ job_id: jobId, closing_time: closing });
        } else if (current?.stage_id === finalStageId) {
    ...
    <div>
      ...
      >
        {grouped.map(({ stage, items }) => (
          <div
            key={stage.id}
            className="rounded overflow-hidden flex flex-col max-h-[calc(100vh-8rem)]"
          ...
          >
            ...
            <div className="bg-purple-50 p-2 flex-1 overflow-y-auto">
              {items.map((t) => (
                <div
                  key={t.id}
