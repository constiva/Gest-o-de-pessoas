...
function CenteredLabel({ x, y, height, width, payload }: any) {
  if (!payload) return null;
  const midY = y + height / 2;
  // `x` from Recharts represents the axis zero point for our custom bar,
  // so we can use it directly to center the label.
  const centerX = x;
  return (
      <text
        x={centerX}
