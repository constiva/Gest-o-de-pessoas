...
export default function JobMetrics({ jobId }: Props) {
  ...
  const [timeToFill, setTimeToFill] = useState<number | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const finalStageId = stages && stages.length ? stages[stages.length - 1].id : null;
        ...
        Object.entries(datesByApp).forEach(([appId, records]) => {
          const ins = records
            .filter((r: any) => r.day_in)
            .map((r: any) => new Date(r.day_in).getTime());
          const first = ins.length ? Math.min(...ins) : null;
          const finalRecord = records.find(
            (r: any) => r.stage_id === finalStageId && r.day_in
          );
          if (first !== null && finalRecord) {
            const last = new Date(finalRecord.day_in).getTime();
        ...
        setTimeToFill(fillCount ? totalFill / fillCount : null);
