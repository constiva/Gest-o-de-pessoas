    ...
    const load = async () => {
      try {
        ...
        Object.entries(datesByApp).forEach(([appId, records]) => {
          const ins = records.map(
            (r: any) => new Date(`${r.day_in}T00:00:00`).getTime()
          );
          const outs = records
            .map((r: any) =>
              r.day_out ? new Date(`${r.day_out}T00:00:00`).getTime() : null
            )
          ...
          records.forEach((r: any) => {
            if (!r.day_in) return;
            const start = new Date(`${r.day_in}T00:00:00`).getTime();
            const todayStart = new Date();
            todayStart.setHours(0, 0, 0, 0);
            const end = r.day_out
              ? new Date(`${r.day_out}T00:00:00`).getTime()
              : todayStart.getTime();
