  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const finalStage = stages && stages[stages.length - 1];
        let closing: number | null = null;
        if (finalStage && job?.created_at) {
          const entries = (dateRows || []).filter(
            (d) => d.stage_id === finalStage.id && !d.day_out && d.day_in
          );
          if (entries.length) {
            const earliest = entries.reduce((m, r) => {
              const t = new Date(r.day_in).getTime();
              return t < m ? t : m;
            }, Infinity);
            if (earliest !== Infinity) {
              closing =
                (earliest - new Date(job.created_at).getTime()) / 86400000;
            }
          }
        }
