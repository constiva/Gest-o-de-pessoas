      ...
      try {
        ...
        if (appIds.length) {
          const { data: hist } = await supabase
            ...
            .select('application_id,from_stage,to_stage,moved_at')
        ...
        (apps || []).forEach((app) => {
          const records = (historyByApp[app.id] || []).sort(
            (a, b) => new Date(a.moved_at).getTime() - new Date(b.moved_at).getTime()
          );
          if (records.length > 1) {
            const first = new Date(records[0].moved_at);
            const last = new Date(records[records.length - 1].moved_at);
            totalFill += (last.getTime() - first.getTime()) / 86400000;
            fillCount++;
          }
          for (let i = 0; i < records.length - 1; i++) {
            const cur = records[i];
            const next = records[i + 1];
            const stageId = cur.to_stage;
            const duration =
              (new Date(next.moved_at).getTime() -
                new Date(cur.moved_at).getTime()) /
              86400000;
            const st = stats[stageId] || {
              total: 0,
              count: 0,
              slaHits: 0,
              slaTotal: 0,
            ...
            st.total += duration;
            st.count++;
            const sla = stageMap.get(stageId)?.sla;
            if (sla) {
              st.slaTotal++;
      ...
      <Card>
        <div className="p-4">
          ...
          <div className="space-y-4 text-sm">
            <div>
              <p className="font-medium">Tempo médio para preencher vaga</p>
              <p>{timeToFill.toFixed(1)} dias</p>
            </div>
            <div>
              <p className="font-medium">Tempo médio em cada etapa</p>
              <ul className="ml-4 list-disc">
                {stageDurations.map((s) => (
                  <li key={s.name}>{`${s.name}: ${s.avg.toFixed(1)} dias`}</li>
                ))}
              </ul>
            </div>
            <div>
