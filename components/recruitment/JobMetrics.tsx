  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (finalStageId && job?.created_at) {
          const finals = (dateRows || []).filter(
            (r: any) => r.stage_id === finalStageId && r.day_in && !r.day_out
          );
          if (finals.length) {
            const earliest = Math.min(
              ...finals.map((r: any) => new Date(r.day_in).getTime())
            );
            const jobCreated = new Date(job.created_at).getTime();
            closingTime = (earliest - jobCreated) / 86400000;
          }
        }
        setTimeToFill(closingTime);
