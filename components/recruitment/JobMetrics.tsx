...
export default function JobMetrics({ jobId }: Props) {
  ...
  const [jobAge, setJobAge] = useState<number | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (job?.created_at) {
          age = (Date.now() - new Date(job.created_at).getTime()) / 86400000;
        }
      ...
      <Card>
        <div className="p-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <div className="p-4">
                <h3 className="font-medium">Idade da vaga</h3>
                {jobAge !== null ? (
                  <p className="text-2xl font-semibold text-center">
                    {formatDuration(jobAge)}
                  </p>
                ) : (
