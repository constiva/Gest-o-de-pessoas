...
export default function RoteiroTab({ jobId }: { jobId: string }) {
  ...
  async function loadData() {
    const { data: job } = await supabase
      .from('jobs')
      .select('company_id,title,script_template_id')
      .eq('id', jobId)
      .single()
    if (job) {
      setCompanyId(job.company_id)
      setJobTitle(job.title)
      const templates = await loadTemplates(job.company_id)
      if (job.script_template_id) {
        const tpl = templates.find((t) => t.id === job.script_template_id)
        if (tpl) applyScript(tpl)
      }
    }
  }
  ...
  async function selectScript(id: string) {
    ...
    await supabase
      .from('jobs')
      .update({ script_template_id: s.id })
      .eq('id', jobId)
    setAppliedMsg(`Roteiro ${s.name} aplicado à vaga ${jobTitle}`)
    setTimeout(() => setAppliedMsg(''), 3000)
  }

  async function createScript() {
    if (!name.trim()) return
    if (scripts.some((sc) => sc.name === name.trim())) {
      alert('Já existe um roteiro com esse nome')
      return
    }
    ...
    if (cfg) {
      ...
      await supabase
        .from('jobs')
        .update({ script_template_id: cfg.id })
        .eq('id', jobId)
      setAppliedMsg(`Roteiro ${cfg.name} aplicado à vaga ${jobTitle}`)
      setTimeout(() => setAppliedMsg(''), 3000)
    }
  }

  async function deleteScript(id: string) {
    await supabase.from('job_script_configs').delete().eq('id', id)
    ...
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <div className="flex items-center gap-1">
          ...
          {currentId && (
            <button
              onClick={() => deleteScript(currentId)}
